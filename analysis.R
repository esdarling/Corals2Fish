...
names(dat)
dat_melt2 <- melt(dat[,c(4,8,12,33:37,29,24,31,27,9)], id.vars = 1:3, variable.name = "CoralResponse") 
head(dat_melt2) 
levels(dat_melt2$CoralResponse)

dat_melt2$CoralResponse <- factor(dat_melt2$CoralResponse, 
                                  levels = c("StressTolerant","Competitive",
                                            "no_genera","Generalist",
                                            "CWMMaxsize","Weedy",
                                            "CWMBrooding","Depth_m",
                                            "CWMFecundity","CWMBranching"))
...
ggplot(data = dat_melt2, aes(x = value, y = Complexity)) +
  ...
  theme(legend.key = element_blank(), 
        ...
        panel.grid.minor = element_blank())     
setwd("/Users/emilydarling/Documents/Work/GitHub/Corals2Fish/1_plots")  
ggsave("complex multiplot.pdf", height= 10, width = 10, units = "in") 
